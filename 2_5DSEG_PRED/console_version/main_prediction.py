...
def orientation_fix(aggregated_volume, ori, post_trans_volume_adjust, size):
    ...
    if ori == "ax":
        aggregated_volume = torch.rot90(
            aggregated_volume, k=2, dims=(-2, -1)
        )  # Inverse rotation transform
        aggregated_volume = einops.rearrange(
            aggregated_volume, "w h d ->  d h w"
        )  # .unsqueeze(0)# Inverse rearrange
        aggregated_volume = post_trans_volume_adjust(aggregated_volume).squeeze(0)

    if ori == "sag":
        aggregated_volume = aggregated_volume.flip(2)
        aggregated_volume = torch.rot90(
            aggregated_volume, k=-1, dims=(-2, -1)
        )  # Inverse rotation transform
        aggregated_volume = post_trans_volume_adjust(aggregated_volume).squeeze(0)
...
def predict_many(cfg: DictConfig) -> None:
    ...
    with torch.no_grad():
        for k, large_val_data in enumerate(pred_loader):
            ...
            assert (
                not volume > 1.3 or not volume < 1
            ), "Resolution of the volume is too high or too low. Please check the resolution of the volume"

            orientations = ["sag", "cor", "ax"]

            for ori in orientations:

                sample_data_val = utils.Minisampler3slice(
                    large_val_data, orientation=ori
                )  # Make 3 slices channels + batch of slices
                sample_loader_val = DataLoader(
                    sample_data_val,
                    batch_size=400,
                    num_workers=NUM_WORKERS,
                    shuffle=False,
                ...
                for val_batch_data in sample_loader_val:
                    ...
                    if lock:
                        ...
                        aggregated_volume = torch.cat(
                            (aggregated_volume, aggregated_volume), dim=0
                        )
                        for j, slice in enumerate(val_outputs):
                            if j > 0:
                                aggregated_volume = torch.cat(
                                    (aggregated_volume, post_trans(slice).cpu())
                                )
                    else:
                ...
                aggregated_volume = torch.cat(
                    (aggregated_volume, aggregated_volume_extra), dim=0
                )
                post_trans_volume_adjust = postprocessing_volume_end(size)

                pred = orientation_fix(
                    aggregated_volume, ori, post_trans_volume_adjust, size
                )
            ...
            if len(orientations) > 1:
                ...
                STAPLE_seg_sitk = sitk.STAPLE(
                    seg_stack, 1.0
                )  # 1.0 specifies the foreground value
            ...
            print("running")
            STAPLE_seg = diameter_opening(
                STAPLE_seg, diameter_threshold=5, connectivity=2
            )
            print("ended")
            STAPLE_seg = STAPLE_seg.astype("int")

            table.add_row(
                f"{patient_num}", f"{round(STAPLE_seg.sum()*volume*0.001, 3)} "
            )
            console.print(table)

            predicted_mask = nib.Nifti1Image(
                STAPLE_seg, affine=img_PROPS.affine, header=img_PROPS.header
            )
            ...
            saved_to_path = (
                f"{path_to_current_subject}/{cfg.data.outputmaskname}.nii.gz"
            )
