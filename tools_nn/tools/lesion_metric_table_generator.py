...
path = "/mnt/CRAI-NAS/all/martinsr/NNunet/tools/plots_and_metrics_wmh/lesion_test/lesion_metrics_25D_test.csv"
pathfp = "/mnt/CRAI-NAS/all/martinsr/NNunet/tools/plots_and_metrics_wmh/lesion_test/lesion_metrics_fp_25D_test.csv"
df25 = pd.read_csv(path)
dffp25 = pd.read_csv(pathfp)

path = "/mnt/CRAI-NAS/all/martinsr/NNunet/tools/plots_and_metrics_wmh/lesion_test/lesion_metrics_bayesian_test.csv"
pathfp = "/mnt/CRAI-NAS/all/martinsr/NNunet/tools/plots_and_metrics_wmh/lesion_test/lesion_metrics_fp_bayesian_test.csv"
dfbay = pd.read_csv(path)
dffpbay = pd.read_csv(pathfp)

path = "/mnt/CRAI-NAS/all/martinsr/NNunet/tools/plots_and_metrics_wmh/lesion_test/lesion_metrics_nnunet_test.csv"
pathfp = "/mnt/CRAI-NAS/all/martinsr/NNunet/tools/plots_and_metrics_wmh/lesion_test/lesion_metrics_fp_nnunet_test.csv"
...
def lesion_brackets(df, dfp, modelname):
    ...
    df["Lesion Sizes [mL]"] = pd.cut(df["Lesion Size"], bins=[0, 0.01, 0.4, 1, 100], labels=["0-0.01", "0.01-0.4", "0.4-1", "1-100"])
    dfp["Lesion Sizes [mL]"] = pd.cut(dfp["Lesion Size"], bins=[0, 0.01, 0.4, 1, 100], labels=["0-0.01", "0.01-0.4", "0.4-1", "1-100"])
...
# add gt lesions
dfmainfp = pd.concat([dffp25, dffpbay, dffpnn], ignore_index=True)

# same figure
f, ax = subplots_centered(nrows=2, ncols=2, figsize=(15,10), nfigs=3)
sns.despine(bottom=True, left=True)
# Show each observation with a scatterplot
sns.stripplot(data = dfmain, x="Recall", y="Lesion Sizes [mL]", hue="Model", dodge=True, alpha=.25, zorder=1, ax=ax[0])
# Show the conditional means, aligning each pointplot in the
# center of the strips by adjusting the width allotted to each
# category (.8 by default) by the number of hue levels
sns.pointplot(data = dfmain, x="Recall", y="Lesion Sizes [mL]", hue="Model", dodge=.532, join=False, palette="dark", markers="d", scale=.75, errorbar=None, ax=ax[0])
# Improve the legend put legend outside
handles, labels = ax[0].get_legend_handles_labels()
ax[0].legend(handles=handles[3:], labels=labels[3:], loc='lower center', bbox_to_anchor=(0.5, 1.05))
# b
# Finalize the figure
plt.tight_layout()
sns.despine(bottom=True, left=True)
# Show each observation with a scatterplot
sns.stripplot(data = dfmainfp, x="Precision", y="Lesion Sizes [mL]", hue="Model", dodge=True, alpha=.25, zorder=1, ax=ax[1])
# Show the conditional means, aligning each pointplot in the
# center of the strips by adjusting the width allotted to each
# category (.8 by default) by the number of hue levels
sns.pointplot(data = dfmainfp, x="Precision", y="Lesion Sizes [mL]", hue="Model", dodge=.532, join=False, palette="dark", markers="d", scale=.75, errorbar=None, ax=ax[1])
# Improve the legend put legend outside
handles, labels = ax[1].get_legend_handles_labels()
ax[1].legend(handles=handles[3:], labels=labels[3:], loc='lower center', bbox_to_anchor=(0.5, 1.05))
# Finalize the figure
plt.tight_layout()
...
sns.despine(bottom=True, left=True)
# Show each observation with a scatterplot
sns.stripplot(data = dfmainfp, x="Lesion Size", y="Lesion Sizes [mL]", hue="Model", dodge=True, alpha=.25, zorder=1, ax=ax[2])
# Show the conditional means, aligning each pointplot in the
# center of the strips by adjusting the width allotted to each
# category (.8 by default) by the number of hue levels
sns.pointplot(data = dfmainfp, x="Lesion Size", y="Lesion Sizes [mL]", hue="Model", dodge=.532, join=False, palette="dark", markers="d", scale=.75, errorbar=None, ax=ax[2])


# Improve the legend put legend outside
handles, labels = ax[2].get_legend_handles_labels()
ax[2].legend(handles=handles[4:], labels=labels[4:], loc='lower center', bbox_to_anchor=(0.5, 1.05))
# Finalize the figure
ax[2].set_xscale('log')
ax[2].set_xlabel('Lesion Size [log]')
plt.tight_layout()
plt.savefig('./plots_and_metrics_wmh/lesion_plots/lesion_recall_precision.jpg', dpi=300)
plt.close()
