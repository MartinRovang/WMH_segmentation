    ...
    for j, batch in enumerate(labels):
        ...
        for i in range(0, 5):
            ...
            if i == 1:
                ...
                batch_onehot_final = torch.cat(
                    (batch_onehot_final, batch_onehot), dim=0
                )
            if i == 2:
        ...
        if j == 0:
            ...
            batch_final = torch.cat(
                (batch_final, batch_onehot_final[None, :, :, :]), dim=0
            )
    labels = batch_final
    return labels
...
def main(config: DictConfig):
    ...
    train_files, val_files = utils.dataprocesser(
        config, config.datasets["datafolder"], filename="FLAIR.nii.gz"
    )

    loss_function = FocalTverskyLoss()

    device = torch.device(
        config.hardware["gpu"] if torch.cuda.is_available() else "cpu"
    )
    ...
    if config.structure["model"] == "UUNET":
        model = UUNet(in_channels=3, out_channels=1, init_features=32).to(device)
    ...
    if config.structure["loadoptim"]:
        ...
        optimizer.load_state_dict(
            torch.load(config.structure["optimweights"], map_location=device)
        )

    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
        optimizer,
        mode="min",
        factor=config.scheduelerparams.factor,
        patience=config.scheduelerparams.patience,
        verbose=False,
        threshold=0.0001,
        threshold_mode="rel",
        cooldown=0,
    ...
    for epoch in range(config.hyperparams.epochs):
        ...
        for fast_start_iter, large_batch in enumerate(
            track(train_loader, description=f"Training...")
        ):
            ...
            sample_data = utils.Minisampler3slice(
                large_batch,
            ...
            sample_loader = DataLoader(
                sample_data,
                batch_size=config.structure["mini_batch_size"],
                num_workers=0,
                shuffle=True,
                drop_last=False,
                pin_memory=torch.cuda.is_available(),
            ...
            for i, batch_data_ax in enumerate(sample_loader):
                ...
                testersys.values_standardized(
                    inputs.cpu().numpy()
                )  # Check if correct intensity
                testersys.labels_binary(
                    labels.cpu().numpy()
                ...
                if config.augmentations["mixup"]:
                    inputs, labels = mixup(
                        inputs, labels, alpha=config.augmentations["alpha"]
                    )
        ...
        if epoch % config.structure.val_interval == 0:
            ...
            with torch.no_grad():
                ...
                step = 0
                epoch_loss_eval = 0
                for fast_start_iter, large_val_data in enumerate(
                    track(val_loader, description=f"Validation...")
                ):
                    ...
                    sample_data_val_ax = utils.Minisampler3slice(
                        large_val_data,
                        all_orientations=config.structure["all_orientations"],
                    )
                    sample_loader_val_ax = DataLoader(
                        sample_data_val_ax,
                        batch_size=config.structure["mini_batch_size"],
                        num_workers=0,
                        shuffle=True,
                    ...
                    for val_batch_data_ax in sample_loader_val_ax:
                        ...
                        if config.structure["all_orientations"]:
                            ...
                            val_images = torch.cat(
                                (val_images_ax, val_images_sag, val_images_cor), dim=0
                            )
                            val_labels = torch.cat(
                                (val_label_ax, val_label_sag, val_label_cor), dim=0
                            )
                        ...
                        metric_outputs = get_all_metrics(
                            val_outputs, val_labels, beta=2
                        )
                ...
                if metric_outputs["fbeta"] > best_metric:
                    ...
                    torch.save(
                        model.state_dict(),
                        "best_metric_model_labelmentation2d_dict.pth",
                    )
                    torch.save(
                        optimizer.state_dict(),
                        "best_metric_model_labelmentation2d_dict_optim.pth",
                    )
                ...
                with open("losses.txt", "a") as f:
                    f.write(
                        f'{epoch_loss_values[-1]}|{loss_array_val[-1]}|{metric_outputs["f1"]}|{metric_outputs["fbeta"]}\n'
                    )
