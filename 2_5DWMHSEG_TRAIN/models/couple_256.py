...
class UpBlock(nn.Module):
    ...
    def __init__(self, in_channels1, in_channels2, out_channels, bilinear=True, dropout_p=0.5, spatial_dims: int = 2):
        super().__init__()
        self.up = UpSample(spatial_dims, in_channels1, in_channels2, scale_factor=2, mode="nontrainable", interp_mode = "bilinear")
...
class CopleNet(nn.Module):
    def __init__(
        ...
        # )
        self.aspp = SimpleASPP(
            spatial_dims, ft_chns[3], int(ft_chns[3] / 4), kernel_sizes=[1, 3, 3, 3], dilations=[1, 2, 4, 6]
        )
