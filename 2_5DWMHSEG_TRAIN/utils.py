    ...
    def __init__(self, data, transform=False, all_orientations=True):
        ...
        for key in data:
            if key == "image" or key == "label":
                ...
                reshaped_in = einops.rearrange(
                    border_fixed_image, "b c d h w -> (b w) c h d"
                )

                reshaped_in = correct_input(reshaped_in)

                reshaped_in = reshaped_in.rot90(k=-2, dims=[-2, -1])
                reshaped_in = reshaped_in[
                    self.rem_eight_slices_1 : -self.rem_eight_slices_1, :, :, :
                ]

                if self.all_orientations:
                    reshaped_in_sag = einops.rearrange(
                        border_fixed_image, "b c d h w -> (b d) c h w"
                    )
                    reshaped_in_cor = einops.rearrange(
                        border_fixed_image, "b c d h w -> (b h) c d w"
                    )

                    reshaped_in_sag = correct_input(reshaped_in_sag)
                    reshaped_in_cor = correct_input(reshaped_in_cor)

                    reshaped_in_sag = reshaped_in_sag[
                        self.rem_eight_slices_2 : -self.rem_eight_slices_2, :, :, :
                    ]
                    reshaped_in_cor = reshaped_in_cor[
                        self.rem_eight_slices_3 : -self.rem_eight_slices_3, :, :, :
                    ]
        ...
        if all_orientations:
            sizes = [
                self.data_all["image"]["ax"].shape[0],
                self.data_all["image"]["sag"].shape[0],
                self.data_all["image"]["cor"].shape[0],
            ]
            self.N = np.min(sizes)
        else:
...
def make_dataset(
    ...
    path,
    filename,
    annotfilename="annot.nii.gz",
    fazekas_subjects=False,
    fazekas_scores=False,
):
    ...
    if type(fazekas_subjects) != bool:
        files = [
            {"image": image, "label": label, "fazekas": fazekas}
            for image, label, fazekas in zip(volume, labels, fazekas_labels)
        ]
    else:
        files = [
            {"image": image, "label": label} for image, label in zip(volume, labels)
        ]
    return files


def sort_subjects_based_on_scanners(all_subjects):
    path_txt = "/workspace/scannerID_2.csv"
    data = pd.read_csv(
        path_txt, header=0, error_bad_lines=False, delimiter="\t", encoding="ISO-8859-1"
    )
    ...
    files_not_in_datainfo_file = [
        x for x in all_subjects if x not in all_files_existing
    ]
...
def dataprocesser(
    ...
    if config.datasets["presplit"] == False:
        ...
        all_subjects = [
            ...
            if file not in ["README.md", "desktop.ini", "info"]
        ]
        np.random.seed(1)
        ...
        scanner_based_list, all_subjects, uniques = sort_subjects_based_on_scanners(
            all_subjects
        )
        testdata = []
        console.print(
            "Stratified test data sampling at 10% for each scanner...",
            style="bold cyan",
        )

        # Do the test data sampling, fraction per machine scanner type
        for uniq in uniques:
            all_from_scanner = scanner_based_list.loc[
                scanner_based_list["ManufacturerModelName"] == uniq
            ]
            sampled = all_from_scanner.sample(
                frac=p_test, replace=False, random_state=1
            )
            console.print(
                f"{list(sampled['ManufacturerModelName'])[0]} {len(sampled['ManufacturerModelName'].to_list())}",
                style="bold yellow",
            )
        ...
        valdata = np.random.choice(
            all_subjects, size=int(np.ceil(N_total_after * p_val)), replace=False
        )
        ...
        console.print(
            f'Loading presplit data from experiment: {config.datasets["presplit_path"]}',
            style="bold cyan",
        )
            ...
            for file in os.listdir(os.path.join(path, patient)):
                if "FLAIR" in file and len(Normed) == 0:
                    try:
                        ...
                        newflair = nib.Nifti1Image(
                            FLAIR_data_normalized,
                            affine=FLAIR_loaded.affine,
                            header=FLAIR_loaded.header,
                        )
                        nib.save(
                            newflair, f"{os.path.join(path, patient)}/Normed_F.nii.gz"
                        )
